...
extern "C" const game_info_t GAME_INFO=
    ...
    { { "bg",      { 0x26, 0x0C, 0x3A, 0x4A, 0x0A, 0xCA, 0x20, 0xE3, 0x94, 0xFF, 0xF6, 0x2C, 0xF3, 0x43, 0xA4, 0x3C, 0x0A, 0xE2, 0xD9, 0x03, 0x62, 0xC6, 0xE4, 0xB8, 0xBC, 0x78, 0x04, 0xBC, 0xEA, 0xC2, 0x72, 0x9F, },
      ...
      { "mov",     { 0x1E, 0x29, 0x71, 0xDC, 0x47, 0xA2, 0xEB, 0x71, 0x67, 0xF8, 0xCA, 0x2C, 0x57, 0x5A, 0x50, 0xC5, 0x6B, 0x55, 0x3F, 0xCB, 0x83, 0xA0, 0x67, 0xEC, 0x32, 0xAF, 0xF0, 0x25, 0x31, 0x4B, 0x92, 0x0F, },
                   { 0 } },
...
extern "C" const BYTE MovBlock[256]={
	,
};
...
void decrypt_mov(const BYTE* seed,
	const string&  seed2,
	unsigned char* buff,
	unsigned long  len)
{
...
int Extract(TCHAR* szFileName)
{
	...
	if(!is_mov)
	{
		MessageBox(0,"ﾖｻﾖｧｳﾖmov",0,0);
		UnmapViewOfFile(lpOri);
		CloseHandle(hMap);
		CloseHandle(hFile);
		return -1;
	}
...
//	unsigned char* toc_buff2 = new unsigned char[hdr.toc_len];
	...
	memcpy(toc_buff,pcuroff,hdr.toc_len);
	pcuroff+=hdr.toc_len;
	...
	for (unsigned long i = 0; i < hdr2->entry_count; i++)
	{
		...
		if (is_mov)
		{
			decrypt_mov(mov_seed, string(""), buff, len);
		}
		else 
		{
