...
def ExtTips(nsb):
    ...
    while(nsb.tell()<len(nsb)-1):
        ...
        if opcode==0xd0:
            ...
            elif len(strs[-1])==2 and (strs[-1]=='\x81\x79' or strs[-1]=='\x81\x7A'):
                e_stroff.append(stroff[-1])
                e_strs.append(strs[-1])
        elif opcode==0x8e:
            stroff=[]
            ...
            for i in range(7):
                readarg(nsb)
        elif opcode==0xd8 and argc==3:
            readarg(nsb)
            readarg(nsb)
            e_stroff.append(nsb.tell())
            tipstr=readarg(nsb).replace('\n','\\n')
            e_strs.append(tipstr)
        else:
