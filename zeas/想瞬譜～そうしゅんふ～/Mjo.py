...
_key = ( 0x00, 0x00, 0x00, 0x00, 0x96, 0x30, 0x07, 0x77, 0x2c, 0x61, 0x0e, 0xee, 0xba, 0x51, 0x09, 0x99,
...
def header(ic):
	s = ic.read(16)
	if s == "MajiroObjX1.000\0":
		encrypted = True
	elif s == 'MjPlainBytecode\0':
		encrypted = False
	elif s == 'MajiroObjV1.000\0':
		encrypted = False
	else:
		raise Exception('invalid mjoheader')
	ic.seek(ic.tell() + 8)
	record_count = Util.read_int(ic)
	ic.seek(0x1c + record_count * 8);
	length = Util.read_int(ic)
	return encrypted, length, 0x20 + record_count * 8

def crypt(ic):
	ic.seek(0)
	encrypted, length, offset = header(ic)
	ic.seek(offset)
	result = bytearray(length)
	buffer = bytearray(ic.read())
	keyl = len(_key)
	for i in xrange(min(len(buffer), length)):
		result[i] = buffer[i] ^ _key[i % keyl]
	return result
